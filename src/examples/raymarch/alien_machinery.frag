...
float map(vec3 p) {
  ...
  // modPolar(p.xz, 6.0);
  float b = sdBox(p + vec3(0,pulse,0) + bumps, vec3(0.3, 0.45, 0.3));
...
vec3 plasma(vec2 p) {
  ...
  float c = (cos(10.0 * px + ang + time * PI) + 
             0.5 * sin(10.0 * py + ang + time * PI)) * sin((px + py) * PI * 2.0);
...
void main( void ) {
  ...
  float cy = 0.6 * sin(PI * 2.0 * time*0.2);
  vec3 camPos = lookAt + vec3(0, cy, lookAt.z - 2.0);
  lightPos = lookAt + vec3(0,0, lookAt.z - 5.0);
