...
const Material boxMaterial     = Material(0.3, 0.9, 0.9, 16.0, vec3(1,1,1), 0.85, false);
...
const Material sphere3Material = Material(0.0, 1.0, 1.0, 128.0, vec3(0,0,0), 0.95, false);
const Material worldMaterial   = Material(0.1, 0.8, 0.2, 1.0, vec3(1,.3,.6), 0.35, false);
...
vec3 lighting(vec3 p, vec3 normal, vec3 camPos, Light light, float objId, bool reflectionPass) {
...
void main( void ) {
  ...
  lights[2] = Light(lightPos3, vec3(1.,1.,1.5 + cos(PI*time)*0.5));

  vec3 sunPos = vec3(0.,2.0 + sin(PI*time*0.25)*0.5,-3.);
  ...
  for(int i = 0; i < LIGHTS; i++) {
    sceneColor += clamp(lighting(p, normal, camPos, lights[i], hit.objId, false), 0., 1.);
  }
  ...
  vec3 refCol = vec3(0.);
  for(int i = 0; i < LIGHTS; i++) {
    refCol += clamp(lighting(pRef, normal, pRef, lights[i], hit.objId, true), 0., 1.);  
  }
  ...
  if (TWO_PASSES) {
    ...
    for(int i = 0; i < LIGHTS; i++) {
      refCol2 += clamp(lighting(pRef, refl2, pRef, lights[i], hit.objId, true), 0., 1.);
    }
  }
