...
float map(vec3 p) {
  ...
  modPolar(p.xz, 6.0);
  float b = sdBox(p + vec3(0,0,0) + bumps, vec3(0.3, 0.55, 0.3));
  float c = sdBox(p, vec3(0.2,2.0,0.2) +bumps);
  c = unionRound(b,c, 0.4);
...
float softShadow(vec3 ro, vec3 rd, float start, float end, float k){
  float shade = 1.0;
  float dist = start;
  const int shadowIter = 14;
  float stepDist = end/float(shadowIter);
  for (int i=0; i<shadowIter; i++){
...
vec3 plasma(vec2 p) {
  float px = 10.0 * p.x / (PI * 2.0);
  float py = 10.0 * p.y / (PI * 2.0);
  float ang = atan(p.x-0.5, p.y-0.5);
  float c = (3.0 * cos(10.0 * px + ang + time * PI) + 
             6.0 * sin(10.0 * py + ang + time * PI)) * sin((px + py) * PI * 2.0);
  return sin(vec3(0.9,0.5,0.1) * time * PI + ang * 4.0) * c * exp(c/2.2);
}
...
void main( void ) {
  ...
  rd.yz *= rot2(PI * sin(PI * time * 0.1)/3.0);
  ...
  sceneColor += plasma(p.z*0.1 +  normal.yy * 3.0) * .007*diffuse*ao*shadowcol;
